...
cd semantic
cat part-*.csv| sed 's/\"\"/AAAAAAAAAAA/g' > semantic.csv

# importa datos hacia psq
PGPASSWORD=$password psql -U $user -h $host -d $db_name -c "\copy semantic.rita FROM 'semantic.csv' with null as E'AAAAAAAAAAA' CSV;"
rm -r ../semantic
