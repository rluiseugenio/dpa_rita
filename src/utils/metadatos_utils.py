...
class Linaje_clean_data():
  ...
  # Clase para reunir los metadatos del testing de extract
class Linaje_extract_testing():
    def __init__(self, fecha=0, nombre_task=0, usuario=0, ip_ec2=0, year=0, month=0, task_status=0):
        self.fecha = fecha # time stamp
        self.nombre_task = self.__class__.__name__#nombre_task
        self.usuario = usuario # Usuario de la maquina de GNU/Linux que corre la instancia
...
# Clase para reunir los metadatos del testing de load
class Linaje_load_testing():
    def __init__(self, fecha=0, nombre_task=0, usuario=0, ip_ec2=0, year=0, month=0, task_status=0):
        self.fecha = fecha # time stamp
        self.nombre_task = self.__class__.__name__#nombre_task
        self.usuario = usuario # Usuario de la maquina de GNU/Linux que corre la instancia
...
def EL_testing_extract(record_to_insert):
    '''
    Funcion auxiliar para insertar a metadatos.testing_extract
    '''
    # Conexion y cursor para query
    connection = psycopg2.connect(user = MY_USER, # Usuario RDS
                                 password = MY_PASS, # password de usuario de RDS
    ...
    return print("metadatos.testing_extract insertion Done - PostgreSQL connection is closed")

def EL_testing_load(record_to_insert):
    '''
    Funcion auxiliar para insertar a metadatos.testing_load
    '''
    # Conexion y cursor para query
    connection = psycopg2.connect(user = MY_USER, # Usuario RDS
                                 password = MY_PASS, # password de usuario de RDS
